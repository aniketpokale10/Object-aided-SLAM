...
/* main cost function #1 */
// Cost function to store reprojection error resulting from the values of lambdas
struct LambdaReprojectionError{

	// Constructor
	LambdaReprojectionError(double *X, double *x, double *v, double *K, double w, double *trans, int numEigV) \
		: X_(X), x_(x), v_(v), K_(K), w_(w), trans_(trans), numEigV_(numEigV) {}
...
*/
		...
		for (int i = 0; i < 3; i++)
		{
			...
			for(int j = 0; j < numEigV_; j++)
			{
				P_[i] += T(l[j])*T(v_[(3*j)+i]);
...
// Cost function to regularize the translation estimate, to prevent a huge drift from the initial estimate
struct TranslationDeltaRegularizer{

	// Constructor
	TranslationDeltaRegularizer(double *trans_init, double weight){
		trans_init_[0] = trans_init[0];
		trans_init_[1] = trans_init[1];
		trans_init_[2] = trans_init[2];
		weight_=weight;
	}
	...
	bool operator() (const T* trans, T* residuals) const{
		...
		residuals[0] = T(weight_)*(trans[0]-T(trans_init_[0]));
		residuals[1] = T(weight_)*(trans[1]-T(trans_init_[1]));
		residuals[2] = T(weight_)*(trans[2]-T(trans_init_[2]));

		return true;
	}

	// Initial translation estimate
	double trans_init_[3];
	double weight_;
...
// Cost function to optimize over R,t, given the 3D and 2D keypoints, as in PnPError
struct PnPError{

	// Constructor
	PnPError(double *X, double *x, double *v, double *K, double w, double *l, int numEigV) \
		: X_(X), x_(x), v_(v), K_(K), w_(w), l_(l), numEigV_(numEigV) {}
...
struct cloudPointReprojectionError {

	// Constructor
	cloudPointReprojectionError(double *X, double *x, double *K) \
		: X_(X), x_(x), K_(K) {}
	...
	bool operator() (const T* const rot, const T* trans, T* residuals) const {
		...
		// Compute the residuals (this one works well)
		residuals[0] = (px_ - T(x_[0]));
		residuals[1] = (py_ - T(x_[1]));
...
// Cost function to optimize over R,t and Lambdas, given the 3D and 2D keypoints.
struct FullBAError{

	// Constructor
	FullBAError(double *X, double *x, double *v, double *K, double w, double *l, int numEigV) \
		: X_(X), x_(x), v_(v), K_(K), w_(w), l_(l), numEigV_(numEigV) {}

	// Operator method. Evaluates the cost function and computes Jacobians.
	template <typename T>
	bool operator() (const T* const rot, const T* trans, const T* lambda, T* residuals) const {
		...
		// regularization_term = Sigma(lambdas)
		/*regularization_term = T(0);
		for(int i = 0; i < numEigV_; i++) regularization_term += abs(lambda[i]);*/
		...
		// Compute the residuals (this one works well)
		residuals[0] = sqrt(T(w_))*(px_ - T(x_[0]));// + T(5)*regularization_term);
		residuals[1] = sqrt(T(w_))*(py_ - T(x_[1]));// + T(5)*regularization_term);
		for(int i = 2; i <= 1 + numEigV_; i++) residuals[i] = lambda[i-2];
...
struct PointCloudError{
	...
	// Constructor
	PointCloudError(double *pt, double *pt_obs, double *K, double *rot, double *trans) \
		: P_e(pt), x_pt(pt_obs), K_(K), rot_(rot), trans_(trans) {}

	// Operator method. Evaluates the cost function and computes Jacobians.
	template <typename T>
	bool operator() (const T* P_, T* residuals) const {
		...
		// Rotate the point (and store the result in the same variable)
		// Order of arguments passed: (axis-angle rotation vector (size 3), point (size 3), array where result is to be stored (size 3))
		//Rotation for point cloud points
		T P_rot_[3];
		for(int i=0;i<3;i++){
			P_rot_[i] = T(rot_[i]);
		} 

		ceres::AngleAxisRotatePoint(P_rot_, P_pt,P_pt);

		//Add the translation for point cloud points
		P_pt[0]+= T(trans_[0]);
		P_pt[1]+= T(trans_[1]);
		P_pt[2]+= T(trans_[2]);

		// Project the obtained 3D point point cloud points down to the image, using the intrinsics (K)
		T p_pt[2];
		p_pt[0] = (T(K_[0])*P_pt[0] + T(K_[1])*P_pt[1] + T(K_[2])*P_pt[2])/(T(K_[6])*P_pt[0] + T(K_[7])*P_pt[1] + T(K_[8])*P_pt[2]);
		p_pt[1] = (T(K_[3])*P_pt[0] + T(K_[4])*P_pt[1] + T(K_[5])*P_pt[2])/(T(K_[6])*P_pt[0] + T(K_[7])*P_pt[1] + T(K_[8])*P_pt[2]); 

		// residuals for point cloud points
		residuals[0] = (x_pt[0] - T(p_pt[0]));
		residuals[1] = (x_pt[1] - T(p_pt[1]));
		return true;
	}
